...
import static com.tmax.ast.config.GeneratorIdentifier.blockId;
import static com.tmax.ast.config.GeneratorIdentifier.classId;
import static com.tmax.ast.config.GeneratorIdentifier.importId;
import static com.tmax.ast.config.GeneratorIdentifier.methodCallExprId;
import static com.tmax.ast.config.GeneratorIdentifier.methodDeclarationId;
import static com.tmax.ast.config.GeneratorIdentifier.packageId;
import static com.tmax.ast.config.GeneratorIdentifier.variableDeclarationId;
...
import com.tmax.ast.service.management.BlockService;
import com.tmax.ast.service.management.ClassService;
import com.tmax.ast.service.management.ImportService;
import com.tmax.ast.service.management.MethodService;
import com.tmax.ast.service.management.PackageService;
import com.tmax.ast.service.management.VariableService;
...
public class ConvertService {
    ...
    private void visitAndBuild(Node node, BlockDTO parentBlockDTO) {
        ...
        // 클래스 바로 아래에서 변수를 선언하는 멤버 필드
            ...
            MemberVariableDeclarationDTO mvdDto = variableService.buildVariableDeclInMemberField(
        ...
        //
            ...
            // 함수 및 생성자 선언 시 build
            ClassDTO belongedClassDTO = classService.getClassDTOList().get(classService.getClassDTOList().size() - 1);
            MethodDeclarationDTO mdDto = methodService.buildMethodDeclaration(methodDeclarationId++,
                    blockDTO.getBlockId(),
            ...
            if (!hashcode.isBlank()) {
                ...
                if (returnMapperDTO != null) { // 리턴 타입이 있는 경우에만
                    HashcodeDto returnHashcodeDto = TypeResolverService
                            .generateReturnTypeHashcode((MethodDeclaration) node);
            ...
            blockDTO = parentBlockDTO;
            MethodCallExprDTO mceDto = methodService.buildMethodCallExpr(methodCallExprId++, blockDTO.getBlockId(),
                    node,
                    nodeType);
            // TypeMapper Test code
    ...
    public void visitVariablesAndBuildClassId() {
        // TODO : 선언한 객체 변수가 다른(하위) 클래스 객체를 initialize 했을 때, initializer 타입에 맞는 클래스의 id를
        // 부여할 수 있도록 수정
        // 클래스 member 변수
        for (MemberVariableDeclarationDTO memberVariableDeclarationDTO : getMemberVariableDeclarationDTOList()) {
            ...
            if (!isImport) {
                ...
                if (!isProject) {
                    System.out.println("[visitVariablesAndBuildClassId] : 변수 '" + memberVariableDeclarationDTO.getName()
                            + "'에 대한 클래스를 발견하지 못했습니다.");
                }
            }
            System.out.println();
        ...
        for (StmtVariableDeclarationDTO stmtVariableDeclarationDTO : getStmtVariableDeclarationDTOList()) {
            ...
            if (!isImport) {
                ...
                if (!isProject) {
                    System.out.println("[visitVariablesAndBuildClassId] : 변수 '" + stmtVariableDeclarationDTO.getName()
                            + "'에 대한 클래스를 발견하지 못했습니다.");
                }
            }
            System.out.println();
    ...
    public void visitMethodsAndBuildClassId() {
        for (MethodDeclarationDTO methodDeclarationDTO : getMethodDeclarationDTOList()) {
            ...
            if (!isImport) {
                ...
                if (!isProject) {
                    System.out.println(
                            "[visitMethodsAndBuildClassId] : 리턴 타입 '" + returnMapperDTO.getType()
                                    + "'에 대한 클래스를 발견하지 못했습니다.");
                }
            }
            System.out.println();
            ...
            for (ParameterDTO parameterDTO : parameterDTOList) {
                ...
                if (!checkParameterIfImportPackage(parameterDTO, methodDeclarationDTO.getBlockId())) {
                    if (!checkParameterIfProjectPackage(parameterDTO, methodDeclarationDTO.getBlockId())) {
                        System.out.println(
                                "[visitMethodsAndBuildClassId] : 파라미터 타입 '" + parameterDTO.getType()
                                        + "'에 대한 클래스를 발견하지 못했습니다.");
                    }
                }
                System.out.println();
    ...
    private Long buildTypeClassId(String methodCallNameExpr, Long blockId) {
        List<StmtVariableDeclarationDTO> stmtVariableDeclarationDTOList = variableService
                .getStmtVariableDeclarationDTOList();
    ...
    private boolean checkParameterIfImportPackage(ParameterDTO parameterDTO, Long blockId) {
        System.out.println(
                "[checkParameterIfImportPackage] : Find Class about '" + parameterDTO.getType() + "' parameter");
        ...
        if (imported.size() == 1) {
            System.out.println(
                    "[checkParameterIfImportPackage] : import 에서 '" + imported.get(0).getName() + "'를 발견했습니다.");
            Map<String, String> importMapper = parseImportClassNameOrImportPackageName(imported.get(0));
            if (importMapper.isEmpty()) {
                return false;
            }

            parameterDTO.setTypeClassId(findAndBuildClassInImportPackage(importMapper.get("packageName"),
                    importMapper.get("className"), parameterDTO.getType()));
            return true;
    ...
    private boolean checkParameterIfProjectPackage(ParameterDTO parameterDTO, Long blockId) {
        if (!parameterDTO.getTypeClassId().equals(0L)) {
            System.out
                    .println("[checkParameterIfProjectPackage] : '" + parameterDTO.getType() + "'는 이미 클래스 id를 부여했습니다.");
            return false;
        }
        String parameterTypeName = parameterDTO.getType().replace(">", "").split("<")[0];
        System.out
                .println("[checkParameterIfProjectPackage] : Find Class about '" + parameterDTO.getType()
                        + "' parameter");
        ...
        for (PackageDTO pkg : packaged) {
            for (ClassDTO cls : classService.getClassDTOList()) {
                ...
                if (pkg.getPackageId().equals(cls.getPackageId()) && cls.getName().equals(parameterTypeName)) {
                    System.out.println("[checkParameterIfProjectPackage] : '" + pkg.getName() + "." + cls.getName()
                            + "'에 대한 클래스를 발견하여 '" + parameterDTO.getType() + "'의 class id '" + cls.getClassId()
                            + "'를 부여합니다");
    ...
    private boolean checkReturnIfImportPackage(ReturnMapperDTO returnMapperDTO, Long blockId) {
        System.out
                .println("[checkReturnIfImportPackage] : Find Class about '" + returnMapperDTO.getType()
                        + "' return mapper");
        Long rootBlockId = findRootBlockIdByCurrentBlockId(blockId);
        ...
        if (imported.size() == 1) {
            ...
            returnMapperDTO.setTypeClassId(findAndBuildClassInImportPackage(importMapper.get("packageName"),
                    importMapper.get("className"), returnMapperDTO.getType()));
            return true;
        } else if (imported.size() > 1) {
    ...
    private boolean checkReturnIfProjectPackage(ReturnMapperDTO returnMapperDTO, Long blockId) {
        if (!returnMapperDTO.getTypeClassId().equals(0L)) {
            System.out
                    .println("[checkReturnIfProjectPackage] : '" + returnMapperDTO.getType() + "'는 이미 클래스 id를 부여했습니다.");
            return false;
        }
        String returnTypeName = returnMapperDTO.getType().replace(">", "").split("<")[0];
        System.out
                .println("[checkReturnIfProjectPackage] : Find Class about '" + returnMapperDTO.getType()
                        + "' return mapper");
        ...
        for (PackageDTO pkg : packaged) {
            for (ClassDTO cls : classService.getClassDTOList()) {
                ...
                if (pkg.getPackageId().equals(cls.getPackageId()) && cls.getName().equals(returnTypeName)) {
                    System.out.println("[checkReturnIfProjectPackage] : '" + pkg.getName() + "." + cls.getName()
                            + "'에 대한 클래스를 발견하여 '" + returnMapperDTO.getType() + "'의 class id '" + cls.getClassId()
                            + "'를 부여합니다");
    ...
    private boolean checkVariableIfImportPackage(VariableDeclarationDTO variableDeclarationDTO) {
        ...
        if (variableDeclarationDTO.getVariableType().getChildNodes().size() == 0) {
            System.out
                    .println("[checkVariableIfImportPackage] : '" + variableDeclarationDTO.getVariableType().asString()
                            + "'는 하위 노드가 존재하지 않습니다.");
            return false;
        }
        String variableTypeName = variableDeclarationDTO.getVariableType().getChildNodes().get(0).toString();
        System.out
                .println("[checkVariableIfImportPackage] : Find Class about '" + variableDeclarationDTO.getType() + " "
                        + variableDeclarationDTO.getName() + "' variable");
        ...
        if (imported.size() == 1) {
            System.out
                    .println("[checkVariableIfImportPackage] : import 에서 '" + imported.get(0).getName() + "'를 발견했습니다.");
            ...
            variableDeclarationDTO.setTypeClassId(findAndBuildClassInImportPackage(importMapper.get("packageName"),
                    importMapper.get("className"), variableDeclarationDTO.getName()));
            return true;
    ...
    private boolean checkVariableIfProjectPackage(VariableDeclarationDTO variableDeclarationDTO) {
        if (!variableDeclarationDTO.getTypeClassId().equals(0L)) {
            System.out
                    .println("[checkVariableIfProjectPackage] : '" + variableDeclarationDTO.getVariableType().asString()
                            + "'는 이미 클래스 id를 부여했습니다.");
            return false;
        }
        // 선언한 변수가 속한 패키지를 찾는다
        ...
        String variableTypeName = variableDeclarationDTO.getVariableType().getChildNodes().get(0).toString();
        System.out
                .println("[checkVariableIfProjectPackage] : Find Class about '" + variableDeclarationDTO.getType() + " "
                        + variableDeclarationDTO.getName() + "' variable");
        ...
        for (PackageDTO pkg : packaged) {
            for (ClassDTO cls : classService.getClassDTOList()) {
                ...
                if (pkg.getPackageId().equals(cls.getPackageId()) && cls.getName().equals(variableTypeName)) {
                    System.out
                            .println("[checkVariableIfProjectPackage] : '" + pkg.getName() + "." + cls.getName()
                                    + "'에 대한 클래스를 발견하여 '"
                                    + variableDeclarationDTO.getName() + "'의 class id '" + cls.getClassId()
                                    + "'를 부여합니다");
    ...
    // findRootBlockIdByCurrentBlockId: 소스코드의 최상단 블락의 위치를 반환 (변수가 선언된 블락 id를 가지고, 해당
    // 블락의 최상단 블락을 찾아 리턴)
    ...
    // findImportsByRootBlockIdAndTypeName: 해당 소스파일에서 임포트로 선언한 패키지+클래스를 비교하여 같은
    // 타입(혹은 클래스) 명을 가진 임포트 리스트를 반환
    ...
    // parseImportClassNameOrImportPackageName: import 에서 패키지와 클래스 이름을 분리하여
    // map<string, string> 타입으로 반환
    ...
    // findClassInImportPackage: 프로젝트에 존재하는 다른 패키지를 import 해서 사용중일 때, 패키지에서 선언한 클래스가
    // import package.class;와 동일하면 class id 반환
    private Long findAndBuildClassInImportPackage(String importPackageName, String importClassName, String targetName) {
        ...
        for (PackageDTO packageDTO : packageService.getPackageDTOList()) {
            if (packageDTO.getName().equals(importPackageName)) {
                for (ClassDTO classDTO : classService.getClassDTOList()) {
                    if (classDTO.getPackageId().equals(packageDTO.getPackageId())
                            && classDTO.getName().equals(importClassName)) {
        ...
        System.out
                .println("[findAndBuildClassInImportPackage] : '" + importName
                        + "'에 대한 클래스를 발견하지 못해 class id 부여에 실패했습니다.");
        return 0L;
    }
}
